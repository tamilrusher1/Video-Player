...
async def start(client, message):
    if not await checkdb.is_user_exist(message.from_user.id):
        ...
        name = await client.ask(message.chat.id, "<b>Welcome To botsworldtar player.\n\nIts Time To Create Account On botsworldtar player\n\nNow Send Me Your Business Name Which Show On Website\nEx :- <code>botsworldtar</code></b>")
        if name.text:
        ...
        rm = InlineKeyboardMarkup([[InlineKeyboardButton("✨ Update Channel", url="https://t.me/botsworldtar")]])
...
async def link_start(client, message):
    if not message.text.startswith(LINK_URL):
        return
    link_part = message.text[len(LINK_URL + "?botsworldtar="):].strip()
    try:
